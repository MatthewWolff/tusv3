...
def _set_cost_constraints(mod, R, C, E, n, l, r, c_max):
	...
	for i in range(0, N):
		for j in range(0, N):                           # no cost if no edge exists
			for s in range(0, r):                         # cost is difference between copy number
...
def _set_bp_appearance_constraints(mod, C_bin, W, E, G, n, l):
	...
	for i in range(0, N):
		for j in range(0, N):
			for b in range(0, l): # only 0 if copy num goes from 0 to 1 across edge (i,j)
				mod.addConstr(X[i, j, b] == 2 + C_bin[i, b] - C_bin[j, b] - E[i, j])
	X_bin = _get_3D_bin_rep(mod, X, 3)
	for i in range(0, N):
		for j in range(0, N):
			for b in range(0, l): # set W as bp appearance
				mod.addConstr(W[i, j, b] == 1 - X_bin[i, j, b])
			for s in range(0, l):
				for t in range(0, l): # breakpoint pairs appear on same edge
					mod.addConstr(_get_abs(mod, W[i, j, s] - W[i, j, t]) <= 1 - G[s, t])
	for b in range(0, l):     # breakpoints only appear once in the tree
...
def _set_ancestry_condition_constraints(mod, C_bin, A, W, U, m, N, l):
	...
	for i in range(0, N):
		for j in range(0, N):
			...
			for s in range(0, l):                                 # to bp t appearing in descendant v_j
				for t in range(0, l):
					mod.addConstr(Z[(i, j)][s, t] == 3 - W_node[i, s] - W_node[j, t] - A[i, j] + Y_bin[i, s])
	Phi = _get_gp_arr_cnt_var(mod, m, l)
	for p in range(0, m):
		for b in range(0, l): # Phi[p,s] >= Phi[p,t] constraint only if t appears in ancestor of s and s is never lost
			mod.addConstr(Phi[p, b] == gp.quicksum([ U[p, k] * C_bin[k, b] for k in range(0, N) ]))
		for s in range(0, l):
			for t in range(0, l):
				mod.addConstr(Phi[p, s] >= Phi[p, t] - 1 + gp.quicksum([ (1 - Z_bin[(i, j)][s, t]) for i in range(0, N) for j in range(0, N) ]))

def _set_segment_copy_num_constraints(mod, Gam, C, Q, W, m, n, l, r):
	N = 2*n-1
	for k in range(0, N):
		for b in range(0, l): # define copy num of segment containing breakpoint
			mod.addConstr(Gam[k, b] == gp.quicksum([ Q[b, s] * C[k, l+s] for s in range(0, r) ]))
			mod.addConstr(C[k, b] <= Gam[k, b]) # cp num breakpoint cant exceed cp num of seg containing bp
	for j in range(0, N):
		for b in range(0, l): # copy number of segment containing bp must be at least 1 if bp appears at node j
			mod.addConstr(Gam[j, b] >= gp.quicksum([ W[i, j, b] for i in range(0, N) ]))
...
def _set_bpf_penalty(mod, S, Pi, U, C, Gam):
	...
	for p in range(0, m):
		for b in range(0, l):
			sg_cpnum_est = gp.quicksum([ U[p, k] * Gam[k, b] for k in range(0, N) ])
...
def _get_cost_constraints(R, C, E, n, l, r, c_max):
	N = 2*n-1
	X = {}
	for s in range(0, r): # x_i,j,s is absolute difference for seg s from node i to j if edge (i,j) exists
		X[s] = cvx.Int(N, N)
	...
	for i in range(0, N):
		for j in range(0, N):
			cst.append(R[i, j] >= sum([ X[s][i, j] for s in range(0, r) ]))
...
def _get_bp_appearance_constraints(C_bin, W, E, G, n, l):
	...
	for s in range(0, l):                   # breakpoint pairs must appear on same edge
		for t in range(0, l):
...
def _get_sum_condition_constraints(C_bin, W, U, m, n, l):
	...
	for b in range(0, l):
		W_node[b] = cvx.Int(N)
		for j in range(0, N):
			cst.append(W_node[b][j] == sum([ W[b][i, j] for i in range(0, N) ]))
	...
	for i in range(0, N):        #   at node j. > 0 otherwise
		for j in range(0, N):
			Y[(i, j)] = cvx.Int(l, l)
			for s in range(0, l):
				for t in range(0, l):
	...
	for s in range(0, l):        #   of node where breakpoint t appears
		for t in range(0, l):
			cst.append(D[s, t] == sum([ (1 - Y_bin[(i, j)][s, t]) for i in range(0, N) for j in range(0, N) ]))

	for p in range(0, m):        # cell fraction of breakpoint in parent must be > cell frac in child
		for s in range(0, l):
			for t in range(0, l):
...
def _get_segment_copy_num_constraints(Gam, C, Q, W, m, n, l, r):
	...
	for k in range(0, N):
		for b in range(0, l):
			cst.append(Gam[k, b] == sum([ Q[b, s] * C[k, l+s] for s in range(0, r) ]))

	cst.append(C[:, :l] <= Gam) # copy number of breakpoint cannot exceed copy number of segment containing bp

	for b in range(0, l):   # copy number of segment containing bp must be at least 1 if bp appears at node j
		for j in range(0, N):
			cst.append(Gam[j, b] >= sum([ W[b][i, j] for i in range(0, N) ]))
...
def _bin(X, Y, m, n, x_max):
	...
	Z = {}
	for b in range(0, num_bits): # create binary variables Z
		Z[b] = cvx.Int(m, n)
		cst.append(0 <= Z[b])
		cst.append(Z[b] <= 1)
	for i in range(0, m):
		for j in range(0, n):
			...
			cst.append( sum([ Z[b][i, j] * 2**b for b in range(0, num_bits) ]) == X[i, j] )
			...
			cst.append( Y[i, j] <= sum([ Z[b][i, j] for b in range(0, num_bits) ]) )
